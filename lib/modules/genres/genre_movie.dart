...
class GenreMovie extends StatefulWidget {
  final int genreId;
  const GenreMovie({Key? key,required this.genreId}) : super(key: key);

  @override
  State<GenreMovie> createState() => _GenreMovieState(genreId: genreId);
}

class _GenreMovieState extends State<GenreMovie> {
  final int genreId;
  _GenreMovieState({required this.genreId});
  @override
  void initState() {
    // TODO: implement initState
    super.initState();
    MovieCubit.get(context).getMovieByGenre(genreId);
  }
  @override
    ...
    return BlocConsumer<MovieCubit,MovieStates>(
      listener: (context,state){},
        builder: (context,state){
         var cubit = MovieCubit.get(context).movieByGenre;
        return BuildCondition(
          condition: state is! GetMovieByGenreLoading,
          builder: (context){
            return Container(
                      ...
                      Navigator.push(
                          ...
                          MaterialPageRoute(
                              builder: (context)=>MovieDetailsScreen(
                                id: cubit.movies![index].id,movie: cubit.movies![index]
                              ),
                          ),
                      );
                    },
          ...
          fallback: (context){
            return Center(child: CircularProgressIndicator(color: ColorManager.lightGrey,));
          },
        );
        },
    );
